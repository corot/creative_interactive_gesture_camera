...
Sensor::Sensor(ros::NodeHandle& nh) :
    it_(nh_), device_found(false), proj_helper(NULL)
{
  nh_ = nh;
...
Sensor::~Sensor()
{
  ...
  if (c_node.isSet())
    device_context.unregisterNode(c_node);
  if (d_node.isSet())
    device_context.unregisterNode(d_node);
  if (a_node.isSet())
    device_context.unregisterNode(a_node);
  if (proj_helper)
    delete proj_helper;
}
